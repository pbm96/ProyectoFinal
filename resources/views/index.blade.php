                    ...
                    <div class="form-group col-lg-4 col-md-5">
                        ...
                        <div class="collapse" id="listaCategorias">
                            ...
                            <?php $cat = \Illuminate\Support\Facades\Input::has('categoriasSeleccionadas') ? \Illuminate\Support\Facades\Input::get('categoriasSeleccionadas') : [] ?> @foreach($listaCategorias as $clave=>$categoria)
                                ...
                                <input class="checkbox__input" type="checkbox" id="{{ $categoria->nombre }}"
                                       name="categoriasSeleccionadas[]" value="{{$categoria->id}}"
                                        {{ in_array($categoria->id, $cat) ? 'checked':'' }} />
                                <label class="checkbox__label"
                                       for="{{ $categoria->nombre }}"> {{ $categoria->nombre }}</label> <br>
                            @endforeach
                        </div>
                    </div>
                    <div class="form-group col-lg-4 col-md-5">
                        ...
                        <input id="slider2" type="text" name="slider" onchange="showPrecioValue()" class="span2"
                        ...
                        <div class="card-header imagenes_productos">
                            @if(count($producto->imagen)>0)
                                ...
                                <img src="{{ asset('imagenes/productos/'.$producto->imagen[0]->nombre) }}"
                                     alt="Imagen del producto" style="width:100%" height="200"
